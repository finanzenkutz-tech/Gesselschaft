-- Function to get recently played games for a group
create or replace function get_group_recent_games(p_group_id uuid)
returns table (
  game_name text,
  last_played_at timestamp with time zone,
  play_count bigint,
  last_session_id uuid,
  game_image_url text
)
language plpgsql
security definer
as $$
begin
  return query
  with ranked_sessions as (
    select 
      gs.id,
      gs.game_name,
      gs.played_at,
      gs.game_image_url,
      row_number() over (partition by gs.game_name order by gs.played_at desc) as rn
    from game_sessions gs
    join events e on gs.event_id = e.id
    where e.group_id = p_group_id
  )
  select 
    rs.game_name,
    rs.played_at as last_played_at,
    (select count(*) from game_sessions gs2 join events e2 on gs2.event_id = e2.id where e2.group_id = p_group_id and gs2.game_name = rs.game_name) as play_count,
    rs.id as last_session_id,
    rs.game_image_url
  from ranked_sessions rs
  where rs.rn = 1
  order by rs.played_at desc;
end;
$$;

-- Grant access to authenticated users
grant execute on function get_group_recent_games(uuid) to authenticated;
grant execute on function get_group_recent_games(uuid) to anon;
