                    ...
                    <div className="p-8 space-y-8">
                        {loading ? (
                            ...
                            <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
                                ...
                                <p className="text-slate-500">Datenvergleich fehlgeschlagen.<br />Versuche es später erneut.</p>
                            </div>
                        ) : (
                            <>
                                {/* Shared Inventory */}
                                <div className="space-y-4">
                                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                        <Box className="w-4 h-4" /> Gemeinsame Spiele in der Sammlung ({data.sharedInventory?.length || 0})
                                    </h3>
                                    <div className="flex flex-wrap gap-2">
                                        {data.sharedInventory && data.sharedInventory.length > 0 ? data.sharedInventory.map((game: string) => (
                                ...
                                <div className="space-y-4">
                                    <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 flex items-center gap-2">
                                        <Dice5 className="w-4 h-4" /> Letzte gemeinsame Runden ({data.stats?.totalShared || 0})
                                    </h3>
                                    <div className="space-y-3">
                                        {data.sharedSessions && data.sharedSessions.length > 0 ? data.sharedSessions.map((session: any, idx: number) => {
                                                    ...
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            ...
                                                            <p className="text-[10px] text-slate-400 font-medium">
                                                                {session.played_at ? new Date(session.played_at).toLocaleDateString('de-DE', { day: '2-digit', month: 'short', year: 'numeric' }) : 'Datum unbekannt'}
                                                            </p>
                                                        </div>
                                                    </div>
                                                    {session.winner_id && (
